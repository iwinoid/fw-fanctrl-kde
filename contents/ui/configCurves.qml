...
Item {
    ...
    readonly property string helperScript: decodeURIComponent(Qt.resolvedUrl("../../scripts/fw_helper.py").toString().replace("file://", ""))
    ...
    DataSource {
        ...
        onNewData: function (sourceName, data) {
            ...
            if (command === "get_config")
                root.handleConfig(stdout, stderr, exitCode);
            else
                root.handleResult(stdout, stderr, exitCode);
        }
    }
    ...
    function updateParam(key, value) {
        if (!configData.strategies || !selectedStrategy)
            return;
        if (!configData.strategies[selectedStrategy])
            configData.strategies[selectedStrategy] = {
                speedCurve: []
            };
    ...
    function sortCurrentCurve() {
        ...
        var values = sorted.map(function (p) {
            return {
                temp: p.temp,
                speed: p.speed
            };
    ...
    function removeCurvePoint(index) {
        var pts = getCurvePoints();
        if (pts.length <= 1 || index < 0 || index >= pts.length)
            return;
        pts.splice(index, 1);
        curveVersion++;
        configChanged = true;
    ...
    function removeStrategy(name) {
        if (!configData.strategies || !configData.strategies.hasOwnProperty(name))
            return;
        if (strategyNames.length <= 1)
            return;
        var remaining = strategyNames.filter(function (s) {
            return s !== name;
        });
    ...
    function renameStrategy(oldName, newName) {
        ...
        if (!isValidStrategyName(newName))
            return false;
        if (!configData.strategies || !configData.strategies.hasOwnProperty(oldName))
            return false;
        if (configData.strategies.hasOwnProperty(newName))
            return false;
    ...
    function commitStrategyRename() {
        ...
        if (newName === selectedStrategy)
            return;
        if (!isValidStrategyName(newName)) {
            showStatus(tr("Invalid strategy name: only letters, numbers, '_' and '-' are allowed.", "策略名无效：只能包含字母、数字、下划线和连字符。"), false);
            strategyNameField.text = Qt.binding(function () {
                return root.selectedStrategy;
            });
            return;
        }
    ...
    ColumnLayout {
        ...
        Controls.Label {
            ...
            color: root.statusSuccess ? Kirigami.Theme.positiveTextColor : Kirigami.Theme.negativeTextColor
            font.bold: true
        ...
        Item {
            ...
            Controls.BusyIndicator {
                ...
                running: !root.configLoaded && !root.configLoadError
                visible: !root.configLoaded && !root.configLoadError
            }

            Controls.Button {
                anchors.centerIn: parent
                visible: !root.configLoaded && root.configLoadError
                text: root.tr("Retry", "重试")
                icon.name: "view-refresh"
                onClicked: root.retryLoadConfig()
            }
            ...
            ColumnLayout {
                ...
                RowLayout {
                        ...
                        contentItem: ColumnLayout {
                            ...
                            RowLayout {
                                ...
                                Controls.ToolButton {
                                    ...
                                    Controls.ToolTip {
                                        text: root.tr("Add strategy", "添加策略")
                                    }
                                }
                            }
                            ...
                            ListView {
                                ...
                                delegate: Item {
                                    ...
                                    Controls.ItemDelegate {
                                        id: strategyItem
                                        width: parent.width
                                        highlighted: strategyDelegateContainer.name === root.selectedStrategy
                                        onClicked: root.selectedStrategy = strategyDelegateContainer.name
                                        ...
                                        contentItem: RowLayout {
                                            ...
                                            Controls.Label {
                                                text: strategyDelegateContainer.name
                                                Layout.fillWidth: true
                                                elide: Text.ElideRight
                                                font.bold: strategyDelegateContainer.name === root.configData.defaultStrategy
                                            }

                                            Controls.ToolButton {
                                                icon.name: "list-remove"
                                                visible: root.strategyNames.length > 1
                                                onClicked: root.requestRemoveStrategy(strategyDelegateContainer.name)
                                                Controls.ToolTip {
                                                    text: root.tr("Delete strategy", "删除策略")
                                                }
                                            }
                                        }
                                    }
                    ...
                    Kirigami.AbstractCard {
                        ...
                        contentItem: ColumnLayout {
                            ...
                            spacing: Kirigami.Units.smallSpacing
                            visible: root.selectedStrategy.length > 0

                            Controls.Label {
                                text: root.tr("Strategy Editor", "策略编辑器")
                                font.bold: true
                            }
                            ...
                            GridLayout {
                                ...
                                Controls.TextField {
                                    ...
                                    onEditingFinished: root.commitStrategyRename()
                                }

                                Controls.Label {
                                    text: root.tr("Update Freq (s):", "更新频率(秒):")
                                }
                                Controls.SpinBox {
                                    from: 1
                                    to: 10
                                    value: root.getStrategyValue("fanSpeedUpdateFrequency", 5)
                                    onValueChanged: root.updateParam("fanSpeedUpdateFrequency", value)
                                    Layout.fillWidth: true
                                }

                                Controls.Label {
                                    text: root.tr("Moving Avg (s):", "移动平均(秒):")
                                }
                                Controls.SpinBox {
                                    from: 1
                                    to: 100
                                    value: root.getStrategyValue("movingAverageInterval", 30)
                                    onValueChanged: root.updateParam("movingAverageInterval", value)
                                    Layout.fillWidth: true
                                }
                            }
                            ...
                            RowLayout {
                                Layout.fillWidth: true
                                Controls.Label {
                                    text: root.tr("Speed Curve", "速度曲线")
                                    font.bold: true
                                    Layout.fillWidth: true
                                }
                                Controls.Label {
                                    text: root.tr("Points are sorted by temperature automatically.", "曲线点会按温度自动排序。")
                                    opacity: 0.6
                                    font.pointSize: Kirigami.Theme.smallFont.pointSize
                                }
                                Controls.ToolButton {
                                    icon.name: "list-add"
                                    enabled: root.curveMaxTemp() < 100
                                    onClicked: root.addCurvePoint()
                                    Controls.ToolTip {
                                        text: root.tr("Add point", "添加点")
                                    }
                                }
                            }
                                    ...
                                    Repeater {
                                        ...
                                        delegate: RowLayout {
                                            ...
                                            Controls.TextField {
                                                ...
                                                text: curvePointRow.modelData.temp.toString()
                                                ...
                                                onTextChanged: {
                                                    if (acceptableInput)
                                                        root.updateCurvePoint(curvePointRow.index, "temp", parseFloat(text));
                                                }
                                                onEditingFinished: root.sortCurrentCurve()
                                            }
                                            ...
                                            Controls.TextField {
                                                ...
                                                horizontalAlignment: Text.AlignHCenter
                                                text: curvePointRow.modelData.speed.toString()
                                                ...
                                                onTextChanged: {
                                                    if (acceptableInput)
                                                        root.updateCurvePoint(curvePointRow.index, "speed", parseInt(text));
                                                }
                                            }
                                            ...
                                            Controls.ToolButton {
                                                icon.name: "list-remove"
                                                enabled: root.getCurvePoints().length > 1
                                                onClicked: root.removeCurvePoint(curvePointRow.index)
                                                Controls.ToolTip {
                                                    text: root.tr("Remove point", "删除点")
                                                }
                                            }
                                        }
                                    }
                        ...
                        Controls.Label {
                            anchors.centerIn: parent
                            visible: root.configLoaded && root.selectedStrategy.length === 0
                            text: root.tr("Select a strategy", "选择要编辑的策略")
                            opacity: 0.5
                        }
                    }
                }
                ...
                Kirigami.AbstractCard {
                    ...
                    contentItem: GridLayout {
                        ...
                        Controls.Label {
                            text: root.tr("Default:", "默认:")
                        }
                        Controls.ComboBox {
                            Layout.fillWidth: true
                            model: root.strategyNames
                            currentIndex: root.strategyNames.indexOf(root.configData.defaultStrategy || "")
                            onActivated: function (idx) {
                                if (idx >= 0) {
                                    root.configData.defaultStrategy = root.strategyNames[idx];
                        ...
                        Controls.Label {
                            text: root.tr("Battery:", "电池:")
                        }
                        Controls.ComboBox {
                            Layout.fillWidth: true
                            model: [""].concat(root.strategyNames)
                            currentIndex: root.strategyNames.indexOf(root.configData.strategyOnDischarging || "") + 1
                            onActivated: function (idx) {
                                root.configData.strategyOnDischarging = idx <= 0 ? "" : root.strategyNames[idx - 1];
                                root.configChanged = true;
                            }
                        }
                    }
        ...
        RowLayout {
            ...
            Controls.Button {
                text: root.tr("Save", "保存")
                icon.name: "document-save"
                highlighted: true
                enabled: root.configChanged && !root.saving
                onClicked: root.saveConfigData()
            }
        }
    }
    ...
    Kirigami.Dialog {
        id: addDialog
        title: root.tr("Add New Strategy", "添加新策略")
        ...
        onVisibleChanged: {
            if (visible) {
                newName.clear();
                errorText = "";
                copyFrom.currentIndex = root.strategyNames.indexOf(root.selectedStrategy);
            ...
            RowLayout {
                ...
                Controls.Button {
                    ...
                    onClicked: {
                        ...
                        if (n.length === 0) {
                            addDialog.errorText = root.tr("Name cannot be empty", "名称不能为空");
                            return;
                        }
                        if (root.strategyNames.indexOf(n) >= 0) {
                            addDialog.errorText = root.tr("A strategy with this name already exists.", "同名策略已存在。");
                            return;
                        }
                        root.addStrategy(n, copyFrom.currentText);
                        addDialog.close();
                    }
                }
            }
    ...
    Kirigami.Dialog {
        id: deleteDialog
        title: root.tr("Delete Strategy", "删除策略")
        ...
        ColumnLayout {
            ...
            Controls.Label {
                Layout.fillWidth: true
                text: root.tr("Delete strategy '%1'?", "确定删除策略 '%1' 吗？").arg(deleteDialog.pendingName)
                wrapMode: Text.WordWrap
            }
            RowLayout {
                Layout.fillWidth: true
                ...
                Controls.Button {
                    text: root.tr("Delete", "删除")
                    highlighted: true
                    onClicked: {
                        root.removeStrategy(deleteDialog.pendingName);
                        deleteDialog.close();
                    }
                }
            }
    ...
    Kirigami.Dialog {
        id: discardDialog
        title: root.tr("Discard Changes", "放弃修改")
        preferredWidth: Kirigami.Units.gridUnit * 18
        leftPadding: Kirigami.Units.gridUnit
        rightPadding: Kirigami.Units.gridUnit
        topPadding: Kirigami.Units.gridUnit
        ...
        ColumnLayout {
            ...
            Controls.Label {
                Layout.fillWidth: true
                text: root.tr("Discard unsaved changes and reload?", "未保存的修改将被丢弃，确定重新加载吗？")
                wrapMode: Text.WordWrap
            }
            RowLayout {
                Layout.fillWidth: true
                ...
                Controls.Button {
                    text: root.tr("Discard and Reload", "放弃并重载")
                    highlighted: true
                    onClicked: {
                        discardDialog.close();
                        root.loadConfig();
